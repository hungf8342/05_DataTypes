---
title: "Data Types and Summarizing Data"
...
Welcome to “Data Types and Summarizing Data”. In this video we will discuss different data types and how to explore data using graphical and numerical summaries. 
...
We will motivate our discussion using an example from an earlier video in our course. In the third video we were exploring the following: “is there a link between hypertension and kidney function?” We used systolic blood pressure as a proxy measure for hypertension, and eGFR as our primary outcome to represent kidney function. We also measured other variables of interest such as age, sex, and income. Table \@ref(tab:tbl-Data) displays the first 10 of the 300 participants in the study.
...
NHANES[1:10,] %>% select(-ACRcorrect, -logACR) %>%
  kable(caption="Snapshot of NHANES Data", digits= 1)
```

When examining a new dataset, we should first identify the data type of each variable. The data type will ultimately play a significant role in what types of analyses, exploratory or inferential, are appropriate. There are two broad types of data: qualitative and quantitative. Qualitative data consists of non-numeric attributes or characteristics of a thing or phenomenon while quantitative data consists of numeric attributes or characteristics.  Both qualitative and quantitative data can be broken into sub-types.

- Qualitative data are those that can be grouped into classes or categories and have multiple subtypes, including binary, nominal, and ordinal. 

  - In our example, sex is a binary or dichotomous variable representing biological sex or ‘sex at birth’ with values 0 or 1. Generally, the dataset should have an accompanying data dictionary or other formal document that explains details about study measurements, variables, and values of individual variables.  Documentation would explain that for sex, for example, a value of ‘0’ represents ‘female sex’ and a value of ‘1’ represents ‘male sex’.  This numerical 0-1 representation of a binary variable is referred to as an indicator variable. The binary variable for sex, for example, gives rise to two indicator variables, one for male sex and one for female sex. In this dataset, the indicator variable for male sex is used with the value 1 used for males and the value 0 for females. 
  
  - The column labeled SEQN is a variable representing the subject identifier and is a nominal variable. Nominal variables have groups for which there is no natural ordering.
  
  - Income is an ordinal variable, or one in which there is a natural ordering. the groups have a ranking in that "\$0 to \$24,999" < "\$25,000 to \$54,999"

- The remaining variables are quantitative data of which there are two sub-types, discrete and continuous.

  - Discrete data are isolated or countable, only taking on certain values. There is no value between any two consecutive discrete values. The number of people living in a household is an example of this variable type. There can only be 0, 1, 2, ... people in a household. For a given discrete variable, there may potentially be an infinite number of discrete values.
  
  - Age, SBP, and eGFR are continuous quantitative data. Continuous data are uncountable, and theoretically, between any two given values there exists another value. Be careful though. There will be limits to measurement accuracy for continuous data. Though age is measured in whole numbers in our example, it is still a continuous variable and not discrete, because ages like 39.5 still make sense. Say, however, age was collected in 5-year intervals so we only knew a patient was 35-40. We can in this case consider age to be ordered and discrete, or an ordinal variable. 

Different variable types require different analytic considerations, whether they are the primary outcome of the study, the primary exposure, or variables that have other roles in the study. Knowing the type of each variable is critical to determining the appropriate analytic approach and the type of conclusions that one may draw from statistical analyses of that variable. A quantitative expert can help identify variable types and associated analytic approaches for your study measurements.



# Graphical Summaries

A first step when beginning analysis of a dataset is to use graphical summaries to provide information about the structure of the data. Ideally, you should plot every single variable you will use in your analyses, regardless of how they will be used. Even variables that will only appear in a descriptive table should be plotted to ensure that the numerical summaries (discussed a little later) provided are appropriate.

Histograms are a good starting point for getting to know the distribution of individual quantitative variables. To make a histogram, divide the range of the individual variable of interest into equally sized groups. For each group, which is called a bin, draw a bar which has a height equal to the counts in that bin. Histograms are often used to visualize the distribution of one quantitative continuous variable. Attention should be drawn to overall pattern, shape, center, and spread, and to deviations and outliers from this pattern. In Figure \@ref(fig:fig-hist), the rightmost bottom plot shows a histogram of ACR. You can see that ACR has an extremely large outlier at 21,000 mg/g. The bulk of the histogram is concentrated below ACR levels of 5000 even though the x-axis extends to 21,000. It is up to the researcher on the team with clinical expertise to determine if this is an implausible or invalid value.
...
Even when the outlier is removed, ACR is still very skewed. Skewed variables have a small portion of observed values predominantly to the left or right of the majority of variable values, resulting in a clustering to one side of a histogram. This will be an important consideration in analyses, such as the need to transform or categorize this variable. In some cases, we can apply a mathematical function or categorize a variable to get rid of skewedness.
...
Sometimes, applying the log function to the variable can help with very skewed data. We call this a log transformation.
...
Boxplots are another useful graphical summary for displaying common summary statistics of a variable: the median, the 25th percentile, and the 75th percentile. You can think of a boxplot as a rotated visual summary of a histogram. A single boxplot shows the median, 25th percentile, and 75th percentile rather than the binned distribution of a quantitative, continuous variable. We can plot multiple boxplots side-by-side to compare the distribution summary statistics across different groups of observations. The boxplot x-axis is then the group labels, and the y-axis, which is equivalent to the x-axis in histograms, consists of the continuous variable values. The distance between the 25th and 75th percentile is called the interquartile range, and any data points outside of 1.5 times the interquartile range are plotted individually to highlight their separation from the bulk of the data values.
...
Although a point outside 1.5 times the interquartile range may visually appear as an outlying value, this does not necessarily mean it is an outlier in a clinical sense. Nor is it necessarily problematic for analyses. The clinical expert and the quantitative expert together can decide whether extreme values are problematic and how they should be handled.
...
While histograms and boxplots convey similar and overlapping information about the distribution of a variable, the following boxplot of hypothetical data provides an example where boxplots are limited by what characteristics can be displayed. Consider a boxplot of hypothetical data in Figure \@ref(fig:fig-boxbi).
...
The actual distribution of these data is given in Figure \@ref(fig:fig-histbi). The boxplot does not provide any information that would alert you to the two-peak structure of these data.
...
Violin plots provide the same information as a boxplot, except the box is replaced or augmented with the estimated density of the data. The density curve is essentially a mirrored, smoothed-out version of the y-axis variable's histogram.
...
Below are violin plots for several variables from the NHANES data stratified by sex. Each pair of violin plots shows the distribution of the y-axis variable, colored and separated by sex.
...
In an earlier video in our course, we discussed using scatterplots for exploring relationships between two quantitative variables. Here, we consider four continuous variables: Age, SBP, log(ACR), and eGFR. There are 10 unique pairs of variables (including pairing variables with themselves). For variables paired with themselves, we plot a histogram, and with variables paired with others, we plot a scatterplot with a smoothed trend line passing through the points. In the spaces for redundant variable pairs, we display the correlation between the two variables.
...
- For categorical variables, bar plots are useful to see the number of categories, and the counts or frequencies of each category or the percent. While they look similar, histograms are for visualizing the distribution of continuous data while bar plots are used for distributions of categorical data.
...
After exploring the data graphically, numerical quantities can be an be used to summarize the data. For categorical data, this is typically the count and percent in each category. For quantitative data, these summaries are typically measures of central tendency, the mean or median, and measures of spread, the standard deviation or interquartile range. The mean is a measure of center based on the average of a set of observations, while the median is a measure of center based on the magnitude of the middle value of a set of observations.  An important difference between these measures relates to their handling of outlying values and as a result are usually selected based on the overall shape or distribution of the data.

- If the data are symmetric and there are no extreme data points in the tails of the distribution, then the mean or median is a good summary of central tendency.
...
Once the central location is identified then we describe the spread using the standard deviation when the mean is used, and the interquartile range when the median is used. Similar to the mean, the standard deviation is a measure of spread focused on numerical differences in the magnitude of the observations. The standard deviation is a function of the squared deviations of each individual observation from the mean of the observations. Since it is a measure of spread around the mean, it should only be paired with the mean, and not the median. The interquartile range describes the spread of the middle fifty percent of the data when ordered from smallest to largest and is  typically paired with the median. Below, Table \@ref(tab:tbl-Demo) is one you might see in a research manuscript that describes the individuals in a study. After carefully exploring the data, the appropriate summary statistics for each variable are given.

Notice, for example, that the the median, sd, and mean for SBP are similar for both men and women. In ACR, however, the standard deviation in men is larger than that in women. This implies ACR values are more variable in men than in women. The medians for both genders are also much smaller than their respective means, indicating that there are a few very large values inflating the ACR mean. This matches the skewed distribution of ACR we saw earlier when talking about histograms.
