...
When examining a new dataset, we should first identify the data type of each variable. The data type will ultimately play a significant role in what types of analyses, both exploratory and inferential are appropriate. There are two broad types of data: qualitative and quantitative. Qualitative data consists of non-numeric attributes or characteristics of a thing or phenomenon while quantitative data consists of numeric attributes or characteristics.  Both qualitative and quantitative data can be broken into sub-types.
...
Even when the outlier is removed, ACR is still very skewed. Skewed variables have a small portion of observed values predominantly to the left or right of the majority of variable values, resulting in a clustering to one side of a histogram. This will be an important consideration in analyses, such as the need to transform or categorize this variable. In some cases, we can apply a mathematical function to or categorize a variable to get rid of skewedness.
...
Boxplots are another useful graphical summary for displaying common summary statistics of a variable: the median, the 25th percentile, and the 75th percentile. You can think of a boxplot as a visual summary of a histogram. A single boxplot shows the median, 25th percentile, and 75th percentile rather than the entire distribution of a quantitative, usually continuous variable. We can plot multiple boxplots side-by-side to compare the distribution summary statistics across different groups of observations. The boxplot x-axis is then the group labels, and the y-axis, which is equivalent to the x-axis in histograms, consists of the variable values. The distance between the 25th and 75th percentile is called the interquartile range, and any data points outside of 1.5 times the interquartile range are plotted individually to highlight their separation from the bulk of the data values.
...
Violin plots provide the same information as a boxplot, except the box is replaced or augmented with the estimated density of the data. The density curve is essentially a mirrored, smoothed-out version of the y-axis's histogram.
...
In an earlier video in our course, we discussed using scatterplots for exploring relationships between two quantitative variables. Here, we consider four continuous variables: Age, SBP, log(ACR), and eGFR. There are 10 unique pairs of variables (including pairing variables with themselves). For variables paired with themselves, we plot a histogram, and with variables paired with others, we plot a histogram with a smoothed trend line passing through the points. In the spaces for redundant variable pairs, we display the correlation between the two variables.
...
Notice, for example, that the the median, sd, and mean for SBP are similar for both men and women. In ACR, however, the sd in men is larger than that in women. The medians for both genders are also very different from their respective means. 
